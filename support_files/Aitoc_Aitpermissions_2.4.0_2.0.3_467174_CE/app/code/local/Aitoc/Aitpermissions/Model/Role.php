 ...
 */
?>
<?php if(Aitoc_Aitsys_Abstract_Service::initSource(__FILE__,'Aitoc_Aitpermissions')){ mhrCeBrjDDmrDkgU('82dde091c49410379a65dfe9366569d2'); ?><?php
...
class Aitoc_Aitpermissions_Model_Role
{
    ...
    private function _getCurrentRoleId()
    {
        ...
        return $session->getUser()->getRole()->getId();
    }
    ...
    public function getAllowedStoreIds()
    {
        if (null == $this->_allowedStoreIds)
        {
        	...
        	if ($this->isScopeWebsite())
        	{
                ...
                foreach ($storeCollection as $store)
                {
                    $this->_allowedStoreIds[] = $store->getStoreId();
                }
        	}
        }
