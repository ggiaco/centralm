    ...
    extends Mage_CatalogSearch_Model_Mysql4_Fulltext_Collection
{
    ...
    public function addSearchFilter($query) {
        try {
            ...
            foreach($this->_rel_alg as $code => &$alg) {
            	if(!isset($alg_rel_likes[$code])) $alg_rel_likes[$code] = array();
            ...
            //die((string)$this->getSelect(). "<BR />\nSite is under maintenance.  Please return in a few minutes.");
        } catch(Exception $e) {
    ...
    protected function _appendProfilerColumns($alg_rel_likes) {
        ...
        if (!Mage::getStoreConfigFlag('bss/dev/profiler')
    ...
    public function getSelectCountSql()
    {
        ...
        //Mage::helper('bss')->log((string)$countSelect);
    ...
    protected function _beforeLoad()
    {
    	...
    	/*
        Mage::helper('bss')->log("Sql to Json results.  SQL=". $this->getSelect());
        
        foreach($this->getResource()->getReadConnection()->fetchAll($this->getSelect()) as $r) {
            Mage::helper('bss')->log(json_encode($r));
        }
        */
