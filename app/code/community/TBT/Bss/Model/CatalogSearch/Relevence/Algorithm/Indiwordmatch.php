...
*/
class TBT_Bss_Model_CatalogSearch_Relevence_Algorithm_Indiwordmatch 
		extends TBT_Bss_Model_CatalogSearch_Relevence_Algorithm_Productnamematch {
	
	public function appendRelevenceScore(TBT_Bss_Model_CatalogSearch_Mysql4_Fulltext_Collection &$search_query_collection, 
			&$rel_likes) {
        ...
        foreach($terms as $t) {
            ...
            if($aw_name = (float)Mage::getStoreConfig('bss/aw/name')) {
	        	//@nelkaake Added on Friday October 22, 2010: TODO Enable/Disable partial word matching
	         	if(Mage::helper("bss")->isEnabledPartialWordMatching()) {
           ...
           //@nelkaake Added on Saturday July 17, 2010: 
			...
			//die("added unplural for {$unplural}");
